...
public class ReplayListener extends AbstractListener {
    ...
    public void onClick(InventoryClickEvent e) {
        ...
        if(!(e.getWhoClicked() instanceof Player) || ReplayHelper.replaySessions
                .containsKey((p = (Player) e.getWhoClicked()).getName())) {
            return;
        }
        ...
        if(e.getCurrentItem() != null && e.getCurrentItem().getItemMeta() != null
                && e.getCurrentItem().getItemMeta().getDisplayName() != null && e.getCurrentItem().getType().getId() == 397) {
            String owner = e.getCurrentItem().getItemMeta().getDisplayName().replaceAll("§6", "");

            if(replayer.getNPCList().containsKey(owner)) {
                p.teleport(replayer.getNPCList().get(owner).getLocation());
            }
        }
    }
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        if(!ConfigManager.UPDATE_NOTIFY || !(ReplaySystem.updater.isVersionAvailable() && p.hasPermission("replay.admin"))) {
            return;
        }
