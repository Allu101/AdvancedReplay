...
public class ReplayListCommand extends SubCommand {
    ...
    public boolean execute(CommandSender cs, Command cmd, String label, String[] args) {
        ...
        cs.sendMessage(ReplaySystem.PREFIX + "Available replays: §8(§6" + ReplaySaver.getReplays().size()
                + "§8) §7| Page: §e" + page + "§7/§e" + pagination.getPages());

        pagination.printPage(page, element -> {
            String message = String.format(" §6§o%s    §e%s", (getCreationDate(element) != null
                    ? format.format(getCreationDate(element)) : ""), element);

            ReplayInfo info = DatabaseReplaySaver.getInfo(element);

            if(cs instanceof Player && info != null && info.getCreator() != null) {
                ((Player) cs).spigot().sendMessage(new ComponentBuilder(message)
                        .event(new HoverEvent(Action.SHOW_TEXT, new ComponentBuilder("§7Replay §e§l"
                                + info.getID() + "\n\n§7Created by: §6" + info.getCreator() + "\n§7Duration: §6"
                                + (info.getDuration() / 20) + " §8sec").create()))
    ...
    private Comparator<String> dateComparator() {
        return (s1, s2) -> {
            Date firstCreationDate = getCreationDate(s1);
            Date secondCreationDate = getCreationDate(s2);

            if(firstCreationDate != null && secondCreationDate != null) {
